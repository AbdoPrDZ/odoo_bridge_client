...
class APIRequest<T> {
  ...
  Future<APIResponse<T>> send() async {
    if (log) {
      dev.log("[ODOO_BRIDGE_CLIENT.API] reqUrl: $type - $url");
      dev.log("[ODOO_BRIDGE_CLIENT.API] reqHeaders: $headers");
      dev.log("[ODOO_BRIDGE_CLIENT.API] reqData: ${jsonEncode(data)}");
      dev.log("[ODOO_BRIDGE_CLIENT.API] reqFiles: ${files.length} file");
    }
    ...
    try {
      ...
      if (log) {
        dev.log("[ODOO_BRIDGE_CLIENT.API] reqError: No Internet Connection");
      }
